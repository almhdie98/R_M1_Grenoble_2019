...
df %>% 
  group_by(dataset) %>% 
  summarise(mean_x = mean(x), sd_x = sd(x), mean_y = mean(y), sd_y = sd(y), cor(x, y))
```
...
```{r, message=FALSE}
df %>% 
  select(Name,`Population estimate, July 1, 2019[4]`,`Census population, April 1, 2010[5]`) %>% 
  rename(state = Name,
  ...
  geom_point()+
  geom_abline(slope = 1, color = 'yellow')
'
```
...
```{r, message= FALSE}
titre <- rstudio %>% html_nodes("h1 a") %>% html_text()
...
```{r}
rst <- data.frame(auteurs, titre, date)
rst
