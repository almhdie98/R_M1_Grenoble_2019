...
```{r ex1}
df <- flights
ex <- filter(df, dep_time>=1200 & dep_time<=1400)
ex <- select(ex, origin, dest)
```
...
```{r ex2}
ex1 <- filter(df, origin == "LGA" | origin == "JFK")
ex1 <- filter(ex1, dep_time>0000 & dep_time<0100)
ggplot(ex1, aes(dest))+geom_bar()

df %>%
  filter(dep_time>=0 & dep_time<=100) %>%
  filter(origin %in% c("JFK","LGA"))->ex1
table(ex1$origin, ex1$dest)
...
```{r ex3}
df %>%
  ggplot(aes(dep_time, dep_delay))+geom_point()
```
...
```{r ex6}
df %>% 
  summarise(moyenne = mean(arr_delay, na.rm = T), ecart_type = sd(arr_delay, na.rm = T), min = min(arr_delay, na.rm = T), max = max(arr_delay, na.rm = T))
```
...
df %>% 
  ...
  ggplot(aes(x = carrier, y = moyenne)) + geom_col()
...
df %>% 
  ...
  group_by(month) %>% 
  summarise(v_moyenne = mean(vitesse, na.rm = T)) %>% 
  ggplot(aes(x= month, y = v_moyenne)) + geom_point() + geom_smooth()
