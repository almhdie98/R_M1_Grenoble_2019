...
```{r ex1}
iris %>% 
  ggplot(aes(x = Sepal.Length, y = Sepal.Width, color = Species)) + geom_point() -> plot1
...
plot_color + aes(shape = Species) + scale_shape_manual(values = c(4, 3, 1)) + scale_color_manual(values = c("red", "purple", "green"))
...
flights %>% 
  ...
  ggplot(aes(x = mois_jour, y = n, color = origin, group = origin)) + geom_line() + theme_minimal()+ scale_color_brewer(palette = "Set1")
...
mpg %>% 
  ...
  geom_text(aes(x = 2008+0.5, label = manufacturer), hjust = 0)+ theme(legend.position = "none")
```
...
flights %>% 
  ...
  group_by(day, month) %>% 
  summarise(n = n()) %>% 
  ggplot(aes(x = day, y = month, fill = n))+ geom_tile()+
